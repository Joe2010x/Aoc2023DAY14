...
public class Mirrors
{
    ...
    public Mirrors(List<string> lines)
    {
        ...
        Position = Enumerable
            .Range(0, Col)
            .Select(col => Enumerable.Range(0, Row).Select(row => lines[col][row]).ToArray())
            .ToList();
    ...
    private (bool, int, int) IsStablePositions(
        List<char[]> position,
        List<List<char[]>> positionCollections
    )
    {
        for (int i = 0; i < positionCollections.Count; i++)
        {
            if (IsStablePosition(position, positionCollections[i]))
                return (true, i, positionCollections.Count);
        }
        return (false, 0, 0);
    }
    ...
    private bool IsStablePosition(List<char[]> position, List<char[]> targetPosition)
    {
        for (int r = 0; r < Row; r++)
        {
            for (int c = 0; c < Col; c++)
            {
                if (position[r][c] != targetPosition[r][c])
                    return false;
            }
        }
    ...
    public int GetValue(int target)
    {
        ...
        var convertedCircleValue =
            (Math.Max(target, 0)) < CircleStart
                ? Math.Max(target, 0)
                : ((target - CircleStart) % circle + CircleStart);
